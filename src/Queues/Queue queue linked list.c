/**
    [PROGRAM] :  Queue Data Structure Implementation Using linked list(FIFO)
    [AUTHOR]  :  Saddam Arbaa
    ...
    Queue flow FIFO rule first in first out
     ...
     Reference in future :---->
     1. https://youtu.be/-wKhZqEI-vM
     2. https://youtu.be/0KeFUi8IqEY
     3. https://youtu.be/a5k5dLk-650
     4. https://youtu.be/PGWZUgzDMYI
     5. https://youtu.be/PgSrOtEay04
     6. https://youtu.be/AeokgRec3GE
     7. https://youtu.be/N9ztp76dwp8
     8. https://youtu.be/gnYM_G1ILm0
     9. https://youtu.be/EXsR7HXlGLw
     10. https://youtu.be/9nEcSGdX5vY  */
...
/* real or tail of queue variable global declaration
   and initializations to NULL */
...
void enqueue(int element)
{
    ...
    {
        /*
        if already some element are in the linked list(queue)we
        have to add the new node at the end of the list(queue)
        ...
        Tail -> next = newNode;   // right side connection first
        Tail = newNode;          // move Tail one step ahead to point to the new node
    }
...
/**
     ...
     Node in queue (first in first out)  */
...
void dequeue()
{
     ...
     * delete the first one for that we need to update 2 nodes
     * Front
     * Front -> next  */
    else
    {
        ...
        printf("Node --> %d been Dequeued\n",temp -> data); // inform user the job is about to done
        Front = Front -> next; // move Front one step Ahead
        temp -> next = NULL;  // assign temp -> next to NULL(this not have to)
        free(temp);          // now Delete temp using free() C function
    }

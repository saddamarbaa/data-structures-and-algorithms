/**
    [PROGRAM] : Hash Tables and Hash Functions Implementation
                Separate Chaining with Doubly Linked Lists
    [AUTHOR]  :  Saddam Arbaa
    [Email]   :  <saddamarbaas@gmail.com>

     C Program to Implement Hash Table Separate Chaining
...
/* A Doubly linked list node Structure to store data
   (key ,value, next node address, previous node address)*/
struct Node
{
    int key;             /* key filed */
...
int main(int argc, char* argv[])    /* the river Code */
{
    ...
    do
    {
        printf("Hash Table Separate Chaining with Doubly Linked Lists :\n");
...
void insert(int key, int value)
{
	...
	// create the node
    struct Node *newNode = CreateNewNode(key, value); // call function to create new(now node is ready to add)
    ...
    else // else cases
    {
		...
		if (find_index == -1)  /* case when the key not present in table */
        {
            ...
            newNode -> prev = hashArray[hashIndex].Tail;  // right side connection first
            hashArray[hashIndex].Tail-> next = newNode;  // left side connection second
        ...
        else /* case when the key is already present in table */
        {
            ...
            struct Node* node_Update = get_Node(list, find_index); // call get_Node() function
            node_Update -> value = value; // update the node value
...
void remove_Item(int key)
{
	...
	/** Handle all the corner cases blow */
    ...
    else // list at hashIndex is not Empty
    {
        ...
        else /* case when the key is present in table */
        {
          /*
            ...
            struct Node *temp;   // local variable of type struct node declaration */
            temp = list;        // temp is are now pointing to head node

            // if having only one node in linked list delete that node
            // and assign head and tail to NULL
            if(temp -> next == NULL && temp -> prev == NULL && temp -> key == key)
            {
                hashArray[hashIndex].Head = NULL;  // assign head to NULL
                hashArray[hashIndex].Tail = NULL;  // assign tail to NULL
            ...
            /*
            case when the given key its at first
            position  and list have more than one node */
            else if (temp -> key == key)
            {
                /** link changes */

                hashArray[hashIndex].Head = temp -> next;  // right side connection first
                hashArray[hashIndex].Head -> prev = NULL;    // left side connection second
            ...
            beginning  maybe its somewhere in middle or last
            so let search it*/
            while(temp -> key != key)  // loop until find the key
            {
                temp = temp -> next;  // move temp to next node
            }
            // after search if temp is equal to tail is mean
            // the key is found at the last node
            if (temp == hashArray[hashIndex].Tail)
            {
                /** link changes */

                temp -> prev -> next = NULL;             // right side connection first
                hashArray[hashIndex].Tail = temp -> prev; // left side connection first
                temp -> next = NULL;  // assign temp -> next to NULL(this not have to)
                temp -> prev = NULL;  // assign temp -> prev to NULL(this not have to)
                free(temp);          // now Delete temp using free() C function
            }
            // the key is not the last node and not first node

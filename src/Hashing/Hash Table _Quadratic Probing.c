/**
    [PROGRAM] : Hash Tables and Hash Functions Implementation Using
    ...
    (keys must be unique, but the value can be repeated)
     ...
     Reference in future :---->
      ...
      9.  https://youtu.be/rvdJDijO2Ro
      10. https://youtu.be/KqqOXndnvic
      11. https://youtu.be/Rp3KxUdV09Y
      12. https://youtu.be/dxrLtf-Fybk
      13. https://youtu.be/ncHmEUmJZf4 */
...
int main(int argc, char* argv[])    /* the river Code */
{
    ...
    do
    {
        printf("Hash Table Implementation with Quadratic Probing  :\n");
...
/**
  A utility function to insert an element in the hash table(key , value)
    ...
    The difference is that if we to try to insert into a space
    that is filled we would first check 1^1=1 element away
    then 2^2=4 elements away,
    then 3^2=9 elements away
    then 4^2=16 elements away and so on.*
   (currentPosition + (H * H)) % arraySize => Quadratic Probing
   where h = 1, 2, 3, 4 and so on. */

void insert(int key, int value)
{
    int hashIndex, i, H;  /* local variables declaration */

	// first step : get the hash code
	hashIndex = hashFunction(key); // call hashFunction()
	i = hashIndex; //counter i is equal to  hashIndex
	H = 1; // H for Quadratic Probing formula initialize to 1
    ...
    while (hashArray[i].flag == 1)
    {
       ...
       /*
       calculate the new next index of i using quadratic probing formula
       index of i = currentPosition + (H * H)) % arraySize */
       i = (i + (H * H)) % capacity;
       H++; // increment H by to be ready for next prob

       /* case when the hash table is full
        this condition mean we have reached the same element again
       (from where we have began probing) */
       if (i == hashIndex)
       {
           printf("\n Hash table is full!!\n");
           return; // we are done
...
void remove_Item(int key)
{
    ...
    prob through hash array until we reach an empty space
    ...
    while (hashArray[i].flag != 0)
    {
       ...
       /* case when we are in Infinite loop
        this condition mean  that we have reached the same element again
       (from where we have began probing) */
       	if (i == hashIndex) /* case when we are in forever loop*/
        {
            printf("This key does not exist in the hash table!!!\n");
            printf("\n All the has table been searched now we are in case when we are in Infinite loop!!\n");
		    return; // we are done
        }
    }
...
void search_Item(int key)
{
    ...
    while (hashArray[i].flag != 0)
    {
        ...
        /* case when we are in Infinite loop and key is not found
        this condition mean that we have reached the same element again
        (from where we have began probing) */
